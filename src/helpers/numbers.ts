function roundTo(n:number, place:number):number {
  return Number(Math.round(Number(n + "e+" + place)) + "e-" + place);
}
//////////////////////////////////////////////////////
const arrPhone = [
...
export const convertToNumber = (number:number|string, lengthInteger = 20, lengthDecimal = 0, separator = '', end = '', interval = 3, floatingPoint= ''):string => {
  ...
  number = String(String(number).match(new RegExp(`(\\d+).(\\d{0,${lengthDecimal>0?lengthDecimal:"-"}})|(\\d+)`))?.[0])
  return (floatingPoint ? number.replace('.', floatingPoint) : number).replace(new RegExp(re, 'g'), '$&' + separator) + end
}
//////////////////////////////////////////////////////
/////////////////PHONE////////////////////////////////
//////////////////////////////////////////////////////
...
export const toPhone = function (e:any) {
  ...
  let newValue = convertToPhone(value)
...
export const toNumber = function (e:any, separator="", lengthInteger=20, lengthDecimal=0) {
  ...
  let newValue = String(convertToNumber(value, lengthInteger, lengthDecimal, separator))
  e.target.value = newValue
  if (oldValue) {
